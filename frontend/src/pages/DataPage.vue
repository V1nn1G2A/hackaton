...
<<<<<<< HEAD
    ...
    if (b.status === "fulfilled") {
      ...
      if (Array.isArray(b.value.data.estimateTasks))
...
<<<<<<< HEAD
  ...
  try {
    await controlObjectsApi.importJira(
      id,
      structureFile.value,
      worklogFile.value,
      deliveryKey.value.trim(),
    );
...
const taskColumns = [
  ...
  { title: "Due Date", key: "dueDate", width: 110 },
  {
    ...
    width: 90,
    render: (r: any) =>
      (r.dataQualityFlags ?? []).length
        ? h(
            NTag,
            { type: "warning", size: "small", bordered: false },
            { default: () => `⚠ ${r.dataQualityFlags.length}` },
          )
        : null,
  },
];
...
async function importEmployees({ file }: any) {
  ...
  try {
    ...
    message.success(
      `Импортировано: ${data.created} новых, обновлено: ${data.updated}`,
    );
    await loadEmployees();
  } catch (e: any) {
    message.error(e.response?.data?.message ?? "Ошибка импорта");
  } finally {
    empImporting.value = false;
  }
}
  ...
  },
  {
    ...
    render: (r: any) =>
      h(
        ...
        {
          size: "small",
          type: r.active ? "success" : "default",
          bordered: false,
        },
        { default: () => (r.active ? "Активен" : "Неакт.") },
      ),
  },
  {
    title: "Проверка",
...
const reviewItems = computed(() => {
  ...
  (dataQuality.value.unlinkedEstimateTasks ?? []).forEach((e: any) => {
    items.push({
      category: "EstimateTask без Epic",
      type: "EstimateTask",
      id: e.id,
      title: e.title,
      reason: "Не связан с Jira Epic",
      action: "baseline",
    });
  });
  (dataQuality.value.unlinkedEpics ?? []).forEach((e: any) => {
    items.push({
      category: "Epic без EstimateTask",
      type: "JiraEpic",
      id: e.jiraKey,
      title: e.title,
      reason: "Не связан с EstimateTask",
      action: "links",
    });
  });
  (dataQuality.value.tasksWithIssues ?? []).forEach((t: any) => {
    items.push({
      category: "Задачи с проблемами",
      type: "JiraTask",
      id: t.jiraKey,
      title: t.title,
      reason: (t.flags ?? []).join(", "),
      ...
      <div>
        ...
        >
          <span v-if="baseline"
            ...
            {{
              new Date(
                baseline.importedAt ?? baseline.createdAt,
              ).toLocaleDateString("ru")
            }}</span
          >
          <span v-if="epics.length">Jira: {{ epics.length }} эпиков</span>
          <span v-if="reviewItems.length" style="color: #f59e0b"
            >⚠ {{ reviewItems.length }} требует проверки</span
          >
        </div>
      </div>
            ...
            >
              <NUploadDragger style="border-radius: 10px; margin-bottom: 16px">
                <div style="padding: 32px; text-align: center">
                  ...
                  <NText style="font-size: 14px; color: #cbd5e1"
                    >Перетащите файл baseline сюда</NText
                  >
                  <br />
...
<<<<<<< HEAD
          <NCard
            ...
            "
          >
            <div style="font-weight: 600; color: #e2e8f0; margin-bottom: 12px">
              Импорт Jira-выгрузки
            </div>
            <div
              style="
                display: flex;
            ...
            >
              <NInput
                v-model:value="deliveryKey"
                size="small"
                placeholder="Ключ родительской задачи, например MERVEIDEV-235"
                style="width: 260px"
                clearable
              />
              <NUpload
...
>>>>>>> new
            ...
            <!-- Summary -->
            <div
              v-if="epics.length || tasks.length"
              style="display: flex; gap: 16px; font-size: 12px; color: #94a3b8"
            >
              <span
                >Эпиков: <b style="color: #e2e8f0">{{ epics.length }}</b></span
              >
              <span
                >Задач: <b style="color: #e2e8f0">{{ tasks.length }}</b></span
              >
              <span style="color: #22c55e"
                >Связаных эпиков:
                <b>{{ epics.filter((e) => e.estimateTaskId).length }}</b></span
              >
          ...
          <div v-if="epics.length">
            <div
              ...
              "
            >
              <span style="font-weight: 600; color: #e2e8f0; font-size: 13px"
                >Эпики ({{ epics.length }})</span
              >
              <NButton size="small" @click="autoLink"
                >⚡ Авто-связка по ключу</NButton
              >
            </div>
            <NDataTable
              :columns="epicColumns"
            ...
            >
              <NUploadDragger style="border-radius: 10px; margin-bottom: 16px">
                <div style="padding: 32px; text-align: center">
                  <div style="font-size: 32px; margin-bottom: 8px">👥</div>
                  <NText style="font-size: 14px; color: #cbd5e1"
                    >Перетащите ADUsers.xlsx сюда</NText
                  >
                </div>
              </NUploadDragger>
            </NUpload>
      ...
      <NTabPane name="links" tab="🔗 Связки">
        ...
        >
          ...
          <NButton size="small" type="primary" @click="autoLink"
            >⚡ Авто-связка</NButton
          >
        </div>
        ...
        >
          ...
          >
            ...
            >
              {{
                et.linkedEpicCount > 0
                  ? `✓ ${et.linkedEpicCount} Epic`
                  : "Не связан"
              }}
            </NTag>
          </div>
        ...
        <NSpin :show="dqLoading">
          ...
          >
            <span style="font-size: 13px; color: #94a3b8">
              Всего проблем:
              <b style="color: #f59e0b">{{ reviewItems.length }}</b>
            </span>
            <NButton size="small" @click="loadDataQuality">🔄 Обновить</NButton>
          </div>

          <NAlert
            v-if="!reviewItems.length && !dqLoading"
            type="success"
    ...
    >
      <NForm v-if="editTarget" label-placement="top">
        ...
        <NFormItem label="Направление">
          <NSelect
            v-model:value="editForm.direction"
